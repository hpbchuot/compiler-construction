...
Token* getToken(void) {
  ...
  case CHAR_MINUS:
    ...
    readChar(); 
    return token;
  case CHAR_TIMES:  // Kiểm tra dấu "*"
    ln = lineNo;
    cn = colNo;
    readChar();  // Đọc ký tự tiếp theo sau '*'

    // Kiểm tra nếu ký tự tiếp theo là dấu "*" nữa (để nhận diện toán tử mũ "**")
    if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_TIMES)) {
        readChar();  // Đọc ký tự tiếp theo (đã là '*', không cần phải kiểm tra lại)
        return makeToken(SB_POWER, ln, cn);  // Trả về token cho phép toán mũ (**)
    } else {
        return makeToken(SB_TIMES, ln, cn);  // Trả về token cho phép nhân (*)
    }
  case CHAR_SLASH:
